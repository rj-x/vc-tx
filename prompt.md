...
- **Swing detection:** a swing high/low is a fractal — a bar whose high (low) exceeds the highs (lows) of the k bars on each side (k configurable, default 3). Swings are only confirmed k bars after the fact; the tracker must use them accordingly (a swing is not known at the bar where it occurs — no look-ahead here either). **Cross-boundary confirmation (ruled 2026-08-13): the k-bar fractal window carries across the session boundary** — a swing in the final k bars of a session confirms only when the next session's opening bars arrive, with fractal neighbors spanning the gap. The semantics are coherent across the gap: a late extreme the next session trades through was not a swing; one it respects confirms exactly when useful, as opening context. Swings whose window spans the boundary carry a `confirmed_across_gap` tag (diagnostic count in the weekly report).
...
- **H5 is exempt from the standard MTF gating rule** (its setting is, by definition, Context `MARKUP`). Its sole gate: Context TF extended beyond a configurable ATR multiple from its trend mean **in the direction being faded** — for the buying-climax fade, close *above* the mean by the required multiple (an unsigned distance would absurdly permit fading a buying climax while the market is extended *below* its mean); sign-flips in the mirror. Disabled by default.

**Strength scoring:** start each hypothesis at a base score; increment for each supporting bar; decrement for bars that contradict the hypothesis without meeting the refutation condition; require score ≥ threshold at confirmation time. **Evidence semantics (general definition — RULES.md must enumerate per hypothesis):** a *supporting* bar is a structural-core label consistent with the hypothesis's narrative (further qualifying absorption for H3, additional probes holding the signature extreme for H1, no-demand rally attempts for H2, further no-supply bars for H4); a *contradicting* bar is one with close_pos beyond the extreme threshold *against* the hypothesis direction that does not meet the refutation condition. The confirming bar's own evidence delta is applied before its confirmation check (deliberate). An optional configurable strength floor kills deeply negative hypotheses early (default off — expiry handles them; the floor never applies in `CONFIRMED_PENDING_GATE`). Log every transition.
...
- **Gate-blocked confirmations (`CONFIRMED_PENDING_GATE`):** a hypothesis whose confirmation event fires while the gate is closed does not die and does not need to re-confirm. It enters `CONFIRMED_PENDING_GATE`: on each subsequent Signal-TF close, **only** the gate and the refutation conditions are re-evaluated — strength no longer accrues or decays, and the strength-floor kill does not apply. The confirm-window expiry is replaced by a **pending window**: `cfg.pending_gate_max_bars`, default 2× the Context/Signal TF ratio, so the gate is re-checked across at least two Context-TF closes (under the confirm-window expiry the state would be statistically dead — a hypothesis confirming late in its window would get zero or near-zero gate re-checks). If the gate opens within the pending window, it graduates at that bar's close. All transitions logged.
- **H5:** exempt (see H5).
